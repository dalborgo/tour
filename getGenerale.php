...
$dr=query("SELECT *, SUM(guadagno) as guad, MAX(tappa) as tp, SUM(tornei) as tor FROM `tt_generale` JOIN tt_player ON tt_generale.nick = tt_player.nick LEFT JOIN tt_squadra ON tt_player.squadra = tt_squadra.nome GROUP BY tt_generale.nick order by guad desc");
